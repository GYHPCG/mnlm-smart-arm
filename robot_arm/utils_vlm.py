...
{
    "response":"好的，从我的眼睛里我看到了一些东西",
    "thing_name": ["红色方块", "绿色方块", "房子简笔画"]
}或者
{
    "response":"嗯，我面前有一些东西，我帮你辨别一下它们是什么东西",
    "thing_name": ["两个红色方块，其中一个大一个小", "一个绿色方块", "房子简笔画"]
}
---
2. 人们让你抓起某个物体或者让你移动到指定位置。你需要提取出该物体的位置和夹爪抓紧的关节度grasp_joint，0度为完全打开夹爪。180为完全关闭。坐标从左上角(0,0)开始，右下角结束，向右为x轴，向下为y轴，并从这张图中分别找到这个物体左上角和右下角的像素坐标，输出json数据结构。例如，如果我的指令是：帮我抓起红色方块。你输出这个的格式：
{
...
---
3. 人们让你把红色方块放在蓝色方块上,夹爪张开130度。你需要从这句话中提取出起始物体和终止物体，并让grasp_joint=130，坐标从左上角(0,0)开始，右下角结束，向右为x轴，向下为y轴，并从这张图中分别找到这两个物体左上角和右下角的像素坐标，输出json数据结构。
...
4. 有时候人们的指令会有点模糊，比如：把东西放到旁边。你需要从这句话中提取出起始物体和终止物体("旁边"这个位置没有指定位置，你自己生成一个合适的位置)，坐标从左上角(0,0)开始，右下角结束，向右为x轴，向下为y轴，并从这张图中分别找到这两个物体左上角和右下角的像素坐标，输出json数据结构。
例如，如果我的指令是：请帮我把红色方块放在旁边（对于""旁边"这个未明确的位置，你自己生成一个在范围内的位置）。
你输出这样的格式：
{
 "thing_name": ["红色方块", "绿色方块", "房子简笔画"],
 "start":"红色方块",
...
def place_to(PROMPT='放到旁边去', input_way='keyboard'):
    '''
    多模态大模型识别图像，吸泵吸取并移动物体
    input_way：speech语音输入，keyboard键盘输入
    '''
    ...
    print('第五步：视觉大模型输出结果后处理和可视化')
    # START_X_CENTER, START_Y_CENTER, END_X_CENTER, END_Y_CENTER = post_processing_viz(result, img_path, check=True)
    place_to_other(result,img_path)
